...
class HBaseContext(@transient sc: SparkContext,
  ...
  def logCredInformation[T] (credentials2:Credentials) {
    ...
    while (it.hasNext) {
      val nextCred = it.next()
      logInfo("getAllTokens:" + nextCred)
    }
  }
    ...
    rdd.foreachPartition(
      it => hbaseForeachPartition[T](
        ...
        (iterator, connection) => {
          ...
          /* Cannot upgrade this to the simple code below, because BufferedMutator in Hbase 1.0
          does not support Increments (yet?)
          ...
          */
          ...
          val mutationList = new util.ArrayList[Mutation]
          iterator.foreach(T => {
            mutationList.add(f(T))
            if (mutationList.size >= batchSize) {
              htable.batch(mutationList, null)
              mutationList.clear()
            }
          })
